...
const Affiliations = () => {
    ...
    const KEY = 'get affiliations' + model;
    ...
    const {
        ...
        staleTime: 0,
        select: (data) => {
            const filteredData = data?.data?.filter(i => i.section_type === HOME_SECTION_TYPES.AFFILIATIONS)
            return filteredData ?? []
        }
    });
    ...
    useEffect(() => {
        if (fetchData && fetchData.length > 0) {
            const affiliationsData = fetchData[0];
            setExistingData(affiliationsData);
            form.setFieldsValue({
                title: affiliationsData.title,
                description: affiliationsData.sub_title,
                buttonText: affiliationsData.button_text,
                navigationLink: affiliationsData.navigation_link,
            });
            setImageUrl(affiliationsData.content?.image || null);
            setIsActive(affiliationsData.is_active || false);
        }
    }, [fetchData, form]);
    ...
    const onFinish = async (formValues: any) => {
        const payload = {
            ...
            section_type: HOME_SECTION_TYPES.AFFILIATIONS,
        };
        if (existingData) {
    ...
    return (
        <div style={{
            ...
            <Title level={3}>Affiliation Section</Title>
                ...
                {isLoading ? (
                    ...
                    >
                        <div style={{
                            ...
                            <Form.Item
                             label={<span style={{ fontWeight: "400", fontSize: 18 }}>Title</span>} 
                             name="title"
