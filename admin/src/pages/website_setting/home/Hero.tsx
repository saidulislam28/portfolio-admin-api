...
const Hero = () => {
    ...
    const {
        ...
        staleTime: 0,
        select: (data) => {
            const filteredData = data?.data?.filter(i => i.section_type === HOME_SECTION_TYPES.HERO)
            return filteredData ?? []
        }
    });
    ...
    useEffect(() => {
        if (fetchData && fetchData.length > 0) {
            ...
            form.setFieldsValue({
                ...
                navigationLink: heroData.navigation_link,
            });
    ...
    const onFinish = async (formValues: any) => {
        const payload = {
            ...
            section_type: HOME_SECTION_TYPES.HERO,
        };
        if (existingData) {
                ...
                {isLoading ? (
                    ...
                    >
                        <div style={{
                            ...
                            >
                                <Input style={{ height: 40 }} placeholder="Hero Title" />
                            </Form.Item>

                            <Form.Item label={<span style={{ fontWeight: "400", fontSize: 18 }}>Description</span>} name="description"
                                rules={[{ required: true, message: 'This field is required' }]}
                            >
                                <Input style={{ height: 40 }} placeholder="Hero description or short title" />
                            </Form.Item>
                        </div>
