...
const Products = () => {
    ...
    const {
        ...
        staleTime: 0,
        select: (data) => {
            const filteredData = data?.data?.filter(i => i.section_type === HOME_SECTION_TYPES.PRODUCTS)
            return filteredData ?? []
        }
    });
    ...
    useEffect(() => {
        if (fetchData && fetchData.length > 0) {
            const productData = fetchData[0];
            setExistingData(productData);
            form.setFieldsValue({
                title: productData.title,
                description: productData.sub_title,
                buttonText: productData.button_text,
                navigationLink: productData.navigation_link,
                number: productData.content?.number
            });
    ...
    const onFinish = async (formValues: any) => {
        const payload = {
            ...
            section_type: HOME_SECTION_TYPES.PRODUCTS,
        };
        if (existingData) {
    ...
    return (
        <div style={{
            ...
            <Title level={3}>Product Section</Title>
                ...
                {isLoading ? (
                    ...
                    >
                        <div style={{
                            ...
                            >
                                <Input  style={{ height: 40 }} placeholder="Product section title" />
                            </Form.Item>
