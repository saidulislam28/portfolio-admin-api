...
const IeltsTestRegistrationForm = () => {
  ...
  useEffect(() => {
    if (isSuccess && settingsResponse?.data) {
      ...
      settingsResponse.data.forEach((item) => {
        if (item.key === "study_abroad_image_1" && item.value) {
        ...
        } else if (item.key === "study_abroad_image_2" && item.value) {
          initialValues[item.key] = [
            {
              uid: "-1",
              name: "current-image",
        ...
        } else if (item.key === "study_abroad_image_3" && item.value) {
          initialValues[item.key] = [
            {
              uid: "-1",
              name: "current-image",
  ...
  const onFinish = async (formValues) => {
    ...
    if (
      formValues.study_abroad_image_2 &&
      formValues.study_abroad_image_2.length > 0
    ) {
      const imageFile = formValues.study_abroad_image_2[0];
      const imageUrl = imageFile.response?.url || imageFile.url || null;
      const imageIndex = dataArray.findIndex(
        (item) => item.key === "study_abroad_image_2"
      );
      if (imageIndex !== -1) {
        dataArray[imageIndex].value = imageUrl;
      }
    ...
    if (
      formValues.study_abroad_image_3 &&
      formValues.study_abroad_image_3.length > 0
    ) {
      const imageFile = formValues.study_abroad_image_3[0];
      const imageUrl = imageFile.response?.url || imageFile.url || null;
      const imageIndex = dataArray.findIndex(
        (item) => item.key === "study_abroad_image_3"
      );
      if (imageIndex !== -1) {
        dataArray[imageIndex].value = imageUrl;
      }
