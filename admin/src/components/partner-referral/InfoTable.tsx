...
export default function ReferralTable({ model, trigger, onClickEdit, ...props }) {
    ...
    const { isLoading, isError, error, isSuccess, data:fetchData, refetch } = useQuery(["Referral Info Table"], () => post(`${API_CRUD_FIND_WHERE}?model=Referral`, { where: { refer_by_partner: Number(id), status:"ACCEPTED" }, include:{User:{select:{first_name:true, last_name:true}}}}, ), { staleTime: 0 });
