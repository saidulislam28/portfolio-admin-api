...
const findBuiltAPK = () => {
    const buildDir = path.join(config.androidPath, 'app', 'build', 'outputs', 'apk');
    ...
    const apkFiles = fs.readdirSync(releaseDir).filter(file => file.endsWith('.apk'));
    ...
    log.success(`Found APK: ${apkFile.name} (${formatFileSize(apkFile.stats.size)})`);
    return apkFile.path;
};
...
const uploadAPK = async (apkInfo) => {
    ...
    if (!config.apiKey) {
        throw new Error('API key not found! Please set APK_API_KEY in your .env file');
    }
    ...
    try {
        ...
        if (response.data.file) {
            ...
            log.info(`Download URL: ${chalk.cyan(config.serverUrl + file.downloadUrl)}`);
        ...
        if (error.response) {
            log.error(`Upload failed: ${error.response.status} - ${error.response.data.error || error.response.statusText}`);
