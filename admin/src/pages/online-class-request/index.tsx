...
const ExamRegistrationPage = () => {
  ...
  const [filters, setFilters] = useState<any>({
    service_type: SERVICE_TYPE.ielts_academic,
    payment_status: "paid",
  });
  ...
  // Filter data based on is_archive status
  ...
  const columns = [
    ...
    },
    {
      title: "Whatsapp",
      key: "whatsapp_number",
      render: (_, record) => `${record?.order_info?.whatsapp_number ?? 'N/A'}`,
    },
    {
      title: "Package Name",
      key: "package",
      render: (_, record) => <Tag color="blue">{record?.Package?.name}</Tag>,
    },
    {
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="small">
          ...
          <Button
            icon={<EyeOutlined />}
            onClick={() => navigate(`/online-course/details/${record.id}`)}
          />
