...
const Features = () => {
    ...
    const {
        ...
        staleTime: 0,
        select: (data) => {
            const filteredData = data?.data?.filter(i => i.section_type === HOME_SECTION_TYPES.FEATURES)
            return filteredData ?? []
        }
    });
        ...
        const payload = {
            ...
            content: formValues.cards.map((card, index) => {
                ...
                return {
                    ...
                    features_description: card.features_description,
                    image: imageUrl,
                };
            }),
    ...
    const onClickEdit = (record: any) => {
        ...
        form.setFieldsValue({
            ...
            cards: record.content.map((item: any) => ({
                ...
                features_description: item.features_description,
                image: [
                    {
                        uid: `${item.id}`,
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                <h1 style={{ fontWeight: 'bold', fontSize: 24 }}>Features Section</h1>
                                    ...
                                    <>
                                        {fields.map(({ key, name, ...restField }) => (
                                            <div key={key} style={{
                                                ...
                                                <Form.Item
                                                    {...restField}
                                                    name={[name, 'features_title']}
                                                    label="Features Title"
                                                    rules={[{ required: true, message: 'Features title is required' }]}
                                                >
                                                    <Input placeholder="Features Title" />
                                                </Form.Item>

                                                <Form.Item
                                                    {...restField}
                                                    name={[name, 'features_description']}
                                                    label="Features Description"
                                                    rules={[{ required: true, message: 'Features description is required' }]}
                                                >
                                                    <Input placeholder="Features Description" />
                                                </Form.Item>
