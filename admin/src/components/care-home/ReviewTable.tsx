...
// @ts-ignore
export default function ReviewTable({ model, trigger, onClickEdit, ...props }) {
    const { id } = useParams();

    const KEY = `all-${model}`;
    const { location } = useParams(); // read id parameter from the url

    const { isLoading, isError, error, isSuccess, data:fetchData, refetch } = useQuery(["Review Details Table"], () => post(`${API_CRUD_FIND_WHERE}?model=Review`, { where: { care_home_id: Number(id) }}), { staleTime: 0 });
